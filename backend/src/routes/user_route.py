...
async def sign_up(signup_user_payload: SignUpUser):
    """
    Sign up route.
    """
    return await user_controllers.signup_user(dict(signup_user_payload))

@user_router.post("/login")
async def log_in(login_user_payload: LoginUser):
    """
    Log in route.
    """
    return await user_controllers.login_user(dict(login_user_payload))
